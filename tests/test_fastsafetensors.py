...
from fastsafetensors import SafeTensorsFileLoader, SafeTensorsMetadata, frameworks
...
from fastsafetensors.st_types import DType, Device
...
def run_nogds_file_read(
    ...
    gbuf = frameworks.OP.alloc_tensor_memory(size, device)
...
def test_load_metadata_and_dlpack(fstcpp_log, input_files) -> None:
    ...
    for input_file in input_files:
        ...
        with safe_open(input_file, framework=frameworks.OP.get_name()) as f:
            for k in f.keys():
...
def test_alloc_gds_buffer(fstcpp_log) -> None:
    ...
    device, _ = get_and_check_device()
    gbuf = frameworks.OP.alloc_tensor_memory(1024, device)
...
def test_memmove(fstcpp_log) -> None:
    ...
    device, _ = get_and_check_device()
    gbuf = frameworks.OP.alloc_tensor_memory(1024, device)
    tmp = frameworks.OP.alloc_tensor_memory(1024, device)
...
def test_nogds_file_reader(fstcpp_log, input_files) -> None:
    ...
    assert fd > 0
    device, dev_is_gpu = get_and_check_device()
    gbuf = frameworks.OP.alloc_tensor_memory(s.st_size, device)
...
def test_NoGdsFileCopier(fstcpp_log, input_files) -> None:
    ...
    with safe_open(input_files[0], framework=frameworks.OP.get_name()) as f:
        for key in tensors.keys():
            ...
            actual = tensors[key]
            assert frameworks.OP.is_equal(actual, exp)
    frameworks.OP.free_tensor_memory(gbuf, device)
...
def test_SafeTensorsFileLoader(fstcpp_log, input_files) -> None:
    ...
    if frameworks.OP.get_name() == "pytorch":
        ...
        raise NotImplementedError(
            f"Do not support the framework: {frameworks.OP.get_name()}"
        )
...
def test_fastsafe_open(fstcpp_log, input_files) -> None:
    ...
    def weight_iterator():
        with fastsafe_open(
            ...
            nogds=True,
            debug_log=True,
            framework=frameworks.OP.get_name(),
        ) as f:
            for k in f.keys():
    ...
    for k, t in weight_iterator():
        assert frameworks.OP.is_equal(t, tensors[k])


def _test_type(tmp_dir, dtype: DType, device: Device) -> None:
    filename = os.path.join(tmp_dir, f"a.safetensors")
    t0 = frameworks.OP.randn((8, 16), device=device, dtype=DType.F32).to(dtype=dtype)
    save_file({f"a": t0.get_raw()}, filename, metadata={"fst": "sample"})
    t2 = {}
    with safe_open(
        filename, framework=frameworks.OP.get_name(), device=device.as_str()
    ) as f:
        for key in f.keys():
    ...
    with fastsafe_open(
        ...
        for key in f.keys():
            ...
            assert frameworks.OP.is_equal(t1, t2[key])


def test_int8(fstcpp_log, tmp_dir) -> None:
    device, _ = get_and_check_device()
    _test_type(tmp_dir, DType.I8, device)

def test_float8_e5m2(fstcpp_log, tmp_dir) -> None:
    device, _ = get_and_check_device()
    _test_type(tmp_dir, DType.F8_E5M2, device)

def test_float8_e4m3fn(fstcpp_log, tmp_dir) -> None:
    device, _ = get_and_check_device()
    _test_type(tmp_dir, DType.F8_E4M3, device)
