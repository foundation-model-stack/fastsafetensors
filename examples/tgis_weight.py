...
def get_config(device_index: int) -> Tuple[bool, int, int]:
    ...
    nogds_str = os.getenv("FST_NOGDS")  # disable GDS if FST_NOGDS==1
    nogds = nogds_str is not None and nogds_str == "1"
...
dtype_convert: Dict[torch.dtype, DType] = {
...
if hasattr(torch, "float8_e5m2"):
    dtype_convert[torch.float8_e5m2] = DType.F8_E5M2
if hasattr(torch, "float8_e4m3fn"):
    dtype_convert[torch.float8_e4m3fn] = DType.F8_E4M3
if hasattr(torch, "uint16"):
    dtype_convert[torch.uint16] = DType.U16
if hasattr(torch, "uint32"):
    dtype_convert[torch.uint32] = DType.U32
if hasattr(torch, "uint64"):
    dtype_convert[torch.uint64] = DType.U64
...
class FastWeights:
    def __init__(
        ...
        rank_filenames: Dict[int, List[str]] = {
        ...
        self._fb = self._loader.copy_files_to_device(
            st_dtype, max_copy_block_size=max_copy_block_size
        )
        self.device = device
        self.st_device = self._loader.device
        self.dtype = dtype
    ...
    def get_tensor(self, tensor_name: str) -> torch.Tensor:
        return self._fb.get_tensor(
            self._get_alias(tensor_name), device=self.st_device, dtype=self.st_dtype
        ).get_raw()

    def push_tensor(self, tensor_name: str, dst_rank: int) -> Optional[torch.Tensor]:
        t = self._fb.push_tensor(
            self._get_alias(tensor_name),
            dst_rank,
            device=self.st_device,
            dtype=self.st_dtype,
        )
        return t.get_raw() if isinstance(t, TensorBase) else None
    ...
    def get_multi_weights_col(self, prefixes: List[str], quantize: str, dim: int):
        if quantize == "gptq":
            ...
            weight = self._fb.get_multi_cols(
                tensor_names, dim, device=self.st_device, dtype=self.st_dtype
            ).get_raw()
        return weight
    ...
    def get_multi_weights_row(self, prefix: str, quantize: str):
        if quantize == "gptq":
            ...
            if self.process_group.size() > 1:
                g_idx: Optional[torch.Tensor] = self.get_tensor(f"{prefix}.g_idx")
    ...
    def _get_gptq_params(self) -> Tuple[int, int]:
        try:
            bits = int(self.get_tensor("gptq_bits").item())
            groupsize = int(self.get_tensor("gptq_groupsize").item())
