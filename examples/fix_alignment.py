...
def fix_sten_file(src_file: str, dst_file: str):
    ...
    print(f"src: filename={src_file}, header_len={meta.header_length}, size={meta.size_bytes}")
    ...
    if dst_header_len % CUDA_PTR_ALIGN > 0:
        ...
        dst_header_str = str.encode(json.dumps(dst_header, separators=(',', ':')), 'utf-8')
        dst_header_len = len(dst_header_str) + 8
        print(f"dst: filename={dst_file}, header_len={dst_header_len} (pad={head_pad}), size={dst_header_len + meta.size_bytes - meta.header_length}")

        dst_fd = os.open(dst_file, os.O_WRONLY|os.O_CREAT|os.O_EXCL, 0o644)
        if dst_fd < 0:
            raise Exception(f"FAIL: open, dst_fd={dst_fd}")
        os_write_full(dst_fd, (dst_header_len - 8).to_bytes(length=8, byteorder='little', signed=False))
        ...
        os_sendfile_full(dst_fd, src_fd, dst_header_len, meta.size_bytes-meta.header_length)
        os.close(dst_fd)
        need_copy = False
...
def os_write_full(fd: int, buf: bytes):
    ...
    while count < len(buf):
        ...
        if c == 0:
            ...
            raise IOError(f"os_write_full: os.write returned error, fd={fd}, len(buf)={len(buf)}, count={count}")
        count += c
...
def os_sendfile_full(src_fd: int, dst_fd: int, offset: int, length: int):
    ...
    while count < length:
        ...
        if c == 0:
            ...
            raise IOError(f"os_sendfile_full: os.sendfile returned error, src_fd={src_fd}, dst_fd={dst_fd}, offset={offset}, length={length}, count={count}")
        count += c

if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("specify a transformers_cache directory, src model name, and dst model name")
        sys.exit(1)
